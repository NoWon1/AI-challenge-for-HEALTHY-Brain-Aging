...
## 2024-05-24 - [Avoid List Comprehensions and `np.column_stack` for Matrix Broadcasting]
**Learning:** Codebase Anti-Pattern/Convention: Avoid using list comprehensions with `np.column_stack` (e.g., `np.column_stack([baseline_surv[np.newaxis, :] ** ph for ph in partial_hazard]).T`) for mathematical operations that could be broadcasted over matrices. These operations drop into python for every single row and are extremely slow for large matrices.
**Action:** Replace list comprehensions iterating row-by-row on vectors and using `np.column_stack` in `neurosaarthi-ad/models/survival/cox_boost.py` with fully vectorized NumPy operations (e.g., `baseline_surv[np.newaxis, :] ** partial_hazard[:, np.newaxis]`). This avoids Python overhead entirely and calculates survival functions in O(1) NumPy operations, giving roughly a ~27x speedup for 10K rows.
## 2024-05-24 - [Avoid Python GroupBy loops for broadcasting group stats]
**Learning:** Codebase Anti-Pattern/Convention: Avoid iterating over pandas `.groupby()` objects with a python `for` loop to extract group statistics and map them to rows (e.g., random effects in `neurosaarthi-ad/models/progression/mixed_effects.py`). This introduces an O(N) pure-Python loop bottleneck.
**Action:** Replace `.groupby()` Python loops for broadcasting with fully vectorized aggregations (or extracting the already-grouped stats into a dictionary/DataFrame). Then use `.reindex()` aligned on the group indicator column to map statistics back to the original dataframe shape for O(1) NumPy/Pandas performance. This yielded roughly ~160x speedup in prediction loops for datasets with large numbers of groups.
## 2026-09-21 - [Vectorize pandas Integrated Brier Score evaluation]
**Learning:** Codebase Anti-Pattern/Convention: Avoid calculating Integrated Brier Score using a python `for` loop over time points combined with multiple calls to `brier_score_loss`, which introduces slow O(M) overhead (where M is the number of time points evaluated).
**Action:** Replace the python loop with a vectorized numpy operation, computing mean squared errors (equivalent to Brier Score in binary cases) and valid masks across all time points at once. This avoids python-level loops entirely and gave roughly ~3.8x speedup. Also handled the deprecation of `np.trapz` by using `np.trapezoid` where available.
