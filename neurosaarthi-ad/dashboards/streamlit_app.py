...
from demo.runtime import PRESET_PROFILES, ParticipantProfile, build_demo_runtime, profile_with
...
def _load_runtime():
    return build_demo_runtime(generate_demo_cohort(seed=42, n_per_cohort=120), n_bootstrap=12)


def _optional_slider(label: str, enabled: bool, default: float | None, minimum: float, maximum: float, step: float, key: str):
    if not enabled:
        return None
...
def _profile_controls() -> ParticipantProfile:
    ...
    st.sidebar.caption("Choose a fictional case, then edit the evidence available to the model.")
    preset_name = st.sidebar.selectbox("Starting case", list(PRESET_PROFILES), key="preset_case")
    ...
    sex = st.sidebar.selectbox("Sex", ["Female", "Male"], index=0 if base.sex == "Female" else 1, key=f"{prefix}-sex")
    education = st.sidebar.slider("Education (years)", 0, 22, int(base.education_years), 1, key=f"{prefix}-education")
    ...
    with st.sidebar.expander("Cognition", expanded=True):
        cognition = st.slider("Cognitive composite (0–30)", 10.0, 30.0, float(base.cognitive_score), 0.1, key=f"{prefix}-cognition")
        memory = st.slider("Memory z-score", -3.0, 2.5, float(base.memory_score), 0.05, key=f"{prefix}-memory")
        executive = st.slider("Executive z-score", -3.0, 2.5, float(base.executive_score), 0.05, key=f"{prefix}-executive")

    with st.sidebar.expander("MRI"):
        mri_available = st.checkbox("MRI features available", value=base.hippocampal_volume_mm3 is not None, key=f"{prefix}-mri-on")
        hippocampal = _optional_slider("Hippocampal volume (mm³)", mri_available, base.hippocampal_volume_mm3, 3500.0, 9000.0, 50.0, f"{prefix}-hippo")
        wmh = _optional_slider("WMH burden (mL)", mri_available, base.wmh_burden_ml, 0.0, 18.0, 0.1, f"{prefix}-wmh")

    with st.sidebar.expander("Blood biochemistry"):
        blood_available = st.checkbox("Blood features available", value=base.hba1c_percent is not None, key=f"{prefix}-blood-on")
        hba1c = _optional_slider("HbA1c (%)", blood_available, base.hba1c_percent, 4.0, 9.0, 0.1, f"{prefix}-hba1c")
        crp = _optional_slider("hs-CRP (mg/L)", blood_available, base.hs_crp_mg_l, 0.1, 12.0, 0.1, f"{prefix}-crp")

    with st.sidebar.expander("OCT / OCTA"):
        oct_available = st.checkbox("Retinal features available", value=base.rnfl_um is not None, key=f"{prefix}-oct-on")
        rnfl = _optional_slider("RNFL thickness (µm)", oct_available, base.rnfl_um, 60.0, 112.0, 0.5, f"{prefix}-rnfl")
        vessel = _optional_slider("Vessel density (%)", oct_available, base.vessel_density_percent, 34.0, 58.0, 0.5, f"{prefix}-vessel")

    with st.sidebar.expander("Genomics · GenomeIndia-aware context"):
        genomics_available = st.checkbox("Genomic features available", value=base.apoe_e4_count is not None, key=f"{prefix}-genomics-on")
        if genomics_available:
            apoe = float(st.select_slider("APOE ε4 allele count", options=[0, 1, 2], value=int(base.apoe_e4_count or 0), key=f"{prefix}-apoe"))
            ancestry = st.slider("Synthetic ancestry PC1", -1.0, 2.0, float(base.ancestry_pc1 or 1.0), 0.05, key=f"{prefix}-ancestry")
        else:
            apoe = None
            ancestry = None
...
def _trajectory_chart(frame: pd.DataFrame) -> alt.Chart:
    base = alt.Chart(frame).encode(
        x=alt.X("year:Q", title="Years from this visit", scale=alt.Scale(domain=[0, 5])),
    )
    band = base.mark_area(color=COLORS["sage"], opacity=0.8).encode(
        y=alt.Y("lower:Q", title="Cognitive composite", scale=alt.Scale(domain=[0, 30])),
        y2="upper:Q",
    )
    line = base.mark_line(color=COLORS["teal"], strokeWidth=3, point=alt.OverlayMarkDef(filled=True, size=65)).encode(
        y="predicted_score:Q",
        tooltip=[alt.Tooltip("year:Q", title="Year"), alt.Tooltip("predicted_score:Q", title="Projected score", format=".1f")],
    )
...
def _driver_chart(frame: pd.DataFrame) -> alt.Chart:
    ...
    return (
        ...
        .encode(
            x=alt.X("effect_points:Q", title="Relative contribution to 3-year estimate (percentage points)"),
            y=alt.Y("modality:N", sort="-x", title=None),
            color=alt.Color(
                "direction:N",
                scale=alt.Scale(domain=["Raises estimate", "Lowers estimate"], range=[COLORS["coral"], COLORS["teal"]]),
                legend=alt.Legend(orient="bottom"),
            ),
            opacity=alt.condition("datum.available", alt.value(1.0), alt.value(0.25)),
            tooltip=["modality:N", "direction:N", alt.Tooltip("effect_points:Q", format="+.1f")],
        )
...
def _twins_chart(forecast) -> alt.Chart:
    ...
    projected = forecast.trajectory[["year", "predicted_score"]].rename(columns={"predicted_score": "cognitive_score"})
    projected["series"] = "Selected profile · projection"
    combined = pd.concat([twins[["year", "cognitive_score", "series"]], projected], ignore_index=True)
    return (
        alt.Chart(combined)
        .mark_line(point=True)
        .encode(
            x=alt.X("year:Q", title="Years from baseline"),
            y=alt.Y("cognitive_score:Q", title="Cognitive composite", scale=alt.Scale(domain=[0, 30])),
            color=alt.Color(
                "series:N",
                scale=alt.Scale(range=[COLORS["teal"], "#7BA69B", "#9BB7AF", "#B8C9C3", "#D5DEDA", COLORS["saffron"]]),
                legend=alt.Legend(orient="bottom", columns=2, title=None),
            ),
            strokeWidth=alt.condition("datum.series === 'Selected profile · projection'", alt.value(4), alt.value(1.6)),
            opacity=alt.condition("datum.series === 'Selected profile · projection'", alt.value(1), alt.value(0.62)),
            tooltip=["series:N", alt.Tooltip("year:Q", format=".1f"), alt.Tooltip("cognitive_score:Q", format=".1f")],
        )
...
def _participant_view(runtime, forecast) -> None:
    ...
    with left:
        ...
        st.caption("The shaded band combines held-out model residuals with dispersion among matched synthetic participants.")
        st.altair_chart(_trajectory_chart(forecast.trajectory), use_container_width=True)
    with right:
        st.markdown("### What moved the estimate")
        st.caption("Transparent modality-level influence relative to the synthetic training population—not a causal explanation.")
    ...
    st.caption("Five nearest longitudinal prototypes after train-only imputation and feature standardisation. Similarity is descriptive, not identity matching.")
    st.altair_chart(_twins_chart(forecast), use_container_width=True)
    twin_table = forecast.twins.copy()
    twin_table["similarity"] = twin_table["similarity"].map(lambda value: f"{value:.0f}%")
    twin_table["baseline_cognition"] = twin_table["baseline_cognition"].map(lambda value: f"{value:.1f}")
    st.dataframe(
        twin_table.rename(
            columns={
...
def _validation_view(runtime) -> None:
    ...
    for column in ("auroc", "auprc", "brier"):
        summary[column] = summary[column].map(lambda value: "—" if pd.isna(value) else f"{value:.3f}")
    st.dataframe(
        summary.rename(
            columns={
    ...
    with calibration_column:
        ...
        observed = (
            ...
            .encode(
                x=alt.X("mean_predicted:Q", title="Mean predicted risk", scale=alt.Scale(domain=[0, 1])),
                y=alt.Y("observed_rate:Q", title="Observed synthetic rate", scale=alt.Scale(domain=[0, 1])),
                size=alt.Size("n:Q", legend=None, scale=alt.Scale(range=[40, 160])),
                tooltip=[alt.Tooltip("mean_predicted:Q", format=".2f"), alt.Tooltip("observed_rate:Q", format=".2f"), "n:Q"],
            )
        )
        ideal = alt.Chart(pd.DataFrame({"x": [0, 1], "y": [0, 1]})).mark_line(strokeDash=[5, 5], color="#9AA9A5").encode(x="x:Q", y="y:Q")
        st.altair_chart((ideal + observed).properties(height=310), use_container_width=True)
    ...
    with ablation_column:
        ...
        chart = (
            ...
            .encode(
                ...
                tooltip=["scenario:N", alt.Tooltip("auprc:Q", format=".3f"), alt.Tooltip("auprc_delta:Q", format="+.3f")],
            )
    ...
    st.caption("Small synthetic subgroups are shown transparently; blanks indicate that a group lacked both outcome classes.")
    ...
    chart = (
        ...
        .encode(
            ...
            color=alt.Color("dimension:N", scale=alt.Scale(range=[COLORS["teal"], COLORS["saffron"], COLORS["coral"], "#759D8F"])),
            column=alt.Column("dimension:N", title=None, header=alt.Header(labelFontSize=13)),
            tooltip=["dimension:N", "group:N", "participants:Q", alt.Tooltip("event_rate:Q", format=".1%"), alt.Tooltip("auprc:Q", format=".3f")],
        )
...
def _harmonisation_view(runtime) -> None:
    ...
    st.caption("The demo intentionally begins with cohort-specific aliases and units, then records every mapping into the common longitudinal model.")
    metrics = st.columns(4)
    metrics[0].metric("Synthetic participants", f"{len(runtime.bundle.tables.participants):,}")
    metrics[1].metric("Longitudinal visits", f"{len(runtime.bundle.tables.visits):,}")
    metrics[2].metric("Mapped source fields", f"{len(runtime.bundle.harmonization_manifest):,}")
    ...
    cohort = filter_columns[0].selectbox("Cohort mapping", ["All"] + list(runtime.bundle.cohort_summary["cohort"]), key="audit_cohort")
    modality = filter_columns[1].selectbox("Modality mapping", ["All"] + sorted(runtime.bundle.harmonization_manifest["modality"].unique()), key="audit_modality")
    ...
    with missing_column:
        ...
        heatmap = (
            ...
            .encode(
                ...
                tooltip=["cohort:N", "modality:N", alt.Tooltip("missing_rate:Q", format=".0%")],
            )
    ...
    with checks_column:
        ...
        st.dataframe(runtime.quality_checks, hide_index=True, use_container_width=True, height=300)
...
def main() -> None:
    st.set_page_config(page_title="NeuroSaarthi-AD · Progression Studio", page_icon="🧠", layout="wide")
    _inject_styles()
